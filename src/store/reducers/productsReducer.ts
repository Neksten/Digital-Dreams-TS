import {
  AddReviewAction, AddReviewActionPayload,
  AxiosProductsAction,
...
export const productsReducer = (state = initialState, action: AppAction): ProductsState => {
  switch (action.type) {
	...
	case ProductActionTypes.ADD_REVIEW:
	  const newProducts = state.products.map((i): IProduct => {
		if (i.id === action.payload.idProduct) {
		  return {...i, reviews: [action.payload.review, ...i.reviews]};
		}
		return i;
	  });
...
export const addReviewReducerAction = (payload: AddReviewActionPayload): AddReviewAction => ({
  type: ProductActionTypes.ADD_REVIEW, payload,
});
